...
function OnLoad()
	...
	Param:addSubMenu("LastHit Settings", "LastHit")
		Param.LastHit:addSubMenu("(E) Spell Settings", "E")
			...
			Param.LastHit.E:addParam("CountHurrican", "How many creeps to kill, with Hurrican :", SCRIPT_PARAM_SLICE, 3, 1, 6)
	-------------------WAVECLEAR|OPTION-------------------------
...
function CustomLoad()
	...
	DelayAction(function()AutoBuy()end, 2)
...
function OnTick()
	...
	-- if TargetHaveBuff("s5tooltip_dragonslayerbuffv1", myHero) then
	-- 	D4 = 0
	-- end
	...
	if Param.KillSteal.Enable then
		for _, unit in pairs(GetEnemyHeroes()) do
			...
			if GetDistance(unit) < SkillQ.range then
				...
				if Param.KillSteal.E and myHero:CanUseSpell(_E) == READY and ValidTarget(unit) and GetDistance(unit) < SkillE.range then
					AutoEHero()
				end
			end
		end
...
-- function OutOfAA()
-- 	for _, unit in pairs(GetEnemyHeroes()) do
-- 		if unit == nil or (unit ~= nil and GetDistance(unit) < myHero.range+myHero.boundingRadius) then
-- 			_G.AutoCarry.MODE_AUTOCARRY = false
-- 			_G.AutoCarry.MODE_LANECLEAR = true
-- 		end
-- 	end
-- end
				...
				if jungleMinion ~= nil then
					if GetStacks(jungleMinion) > 0 and GetDistance(jungleMinion) < SkillE.range then
						...
						if not TargetHaveBuff("summonerexhaust", myHero) then
							if D3 > jungleMinion.health and IsSpecialAMobToE[jungleMinion.name] and Param.Jungle.E.SpecialMob then
								CastSpell(_E)
							end
			...
			if unit ~= nil and GetDistance(unit) < SkillE.range then
				if GetStacks(unit) > 0 then
					...
					if not TargetHaveBuff("summonerexhaust", myHero) then
						...
						if D3 > unit.health and not Immune(unit) then
							CastSpell(_E)
						end
					elseif TargetHaveBuff("summonerexhaust", myHero) then
						if (D3 - ((D3 * 40)/100)) > unit.health and not Immune(unit) then
							CastSpell(_E)
						end
					end
				end
...
function OnGainBuff(source, unit, buff)
	if unit.isMe then
...
SkillW = { name = "Sentinel", range = 5200, delay = 0.25, speed = math.huge, width = 250, ready = false }
SkillE = { name = "Rend", range = 1000, delay = 0.50, speed = nil, width = nil, ready = false }
SkillR = { name = "Fate's Call", range = 1200, delay = nil, speed = nil, width = nil, ready = false }
...
--[[ -- Buff
Trynda R - "Undying Rage"
Zilean R - "Chrono Shift"
Kayle R  - "JudicatorIntervention"
Kindred R  - "kindredrnodeathbuff"
Lissandra R - 
Zhonyas 

Buff : "barontarget" damage reduced by 50%??

Alistar : ferocioushowl ?
...
plusieurs drakes = moins de dégâts

GA = 90% E check creeps, kill creeps E if possible
