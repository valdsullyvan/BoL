...
function LogicQ(unit)
	...
	if unit ~= nil and GetDistance(unit) <= SkillQ.range and myHero:CanUseSpell(_Q) == READY then
		...
		elseif Param.prediction.n1 == 2 then
			local skillshotQ = LineSS(2200,1300,90,625,HUGE)
			local skillshotQ = dp:bindSS("AnviaQ",skillshotQ,50)
			local status,hitPos,perc = dp:predict("AniviaQ",target)
			print(hitPos)
			  if hitPos then 
			  CastSpell(_Q,hitPos.x,hitPos.z)-- end
		elseif Param.prediction.n1 == 3 then
